      ...
      {trips.length === 0 ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          {trips.map(trip => {
            ...
            return (
              <Card key={trip.id} className="glass-card flex flex-col md:flex-row justify-between items-start md:items-center gap-6 p-8 rounded-2xl border-none">
                <CardContent className="p-0 flex-1 min-w-0">
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap', marginBottom: 12 }}>
                    <Badge variant="secondary" className="bg-pink-500/10 text-pink-500 border border-pink-500/20 text-[9px] uppercase font-bold tracking-wider font-mono h-auto px-2 py-0.5 rounded">
                      TRIP ID: {trip.id}
                    </Badge>
                  ...
                  {trip.interests?.length > 0 && (
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                      {trip.interests.map(i => (
                        <Badge key={i} variant="secondary" className="bg-pink-500/10 text-pink-500 border border-pink-500/20 text-[10px] px-2 py-0.5 rounded-full">{i}</Badge>
                      ))}
                    </div>
                  )}
                </CardContent>

                <button onClick={() => onSelect(trip.id)} className="btn-primary" style={{ flex_shrink: 0, padding: '12px 24px', fontSize: 13, borderRadius: 8 }}>
                  View Itinerary →
                </button>
              </Card>
            );
